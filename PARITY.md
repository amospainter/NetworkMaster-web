...
- **Cache/CDN device**: a web-only $130 buildable sink with no native counterpart. Serves same-subnet bulk/stream traffic locally instead of round-tripping to the Cloud Edge (35% base hit chance, upgradeable to 55%); realtime and cross-subnet traffic are unaffected.
- **Wireless repeater**: a web-only $50 buildable that must sit inside a live access point's coverage to activate, then projects a second coverage zone of its own, extending that hub's reach at a one-tick queue-delay cost for clients it serves. Cannot chain off another repeater.
- **Metered income (ISP Hub, Data Center)**: replaces the flat periodic budget allocation with a delivery-metered payout (per-priority cents per packet, capped at 3× the flat rate), rewarding scenarios that frame the player as the service provider.
- **QoS boost**: a free, player-toggled per-device priority boost on any forwarding device, with no native counterpart. Sorts one priority class above everything else on arrival at that device, at the cost of 10% less effective admission capacity while set.
- **SLA contracts**: a periodic, self-balancing latency-or-delivery challenge offered from the top bar once a run has reached its challenge-event window, with no native counterpart. Pays budget on success, costs score on failure — a real stake independent of the core survival loop.
